...
namespace Dacodelaac.Utils
{
    public static class MonoBehaviorExtension
    {
        ...
        /* Dùng toán tử bool của Unity chứ KHÔNG dùng ??=. Unity overload == để object đã Destroy
           so sánh bằng null ("fake null"), nhưng ??= kiểm tra null thật nên sẽ coi object chết là
           cache hợp lệ và trả về nó -> MissingReferenceException. Mảng thì ??= vẫn đúng. */
        public static T GetAndCacheComponentInChildren<T>(this MonoBehaviour mono, ref T cache, bool includeInactive = false) where T : Component
        {
            if (!cache) cache = mono.GetComponentInChildren<T>(includeInactive);
            return cache;
        }
        ...
        public static T GetAndCacheComponentInParent<T>(this MonoBehaviour mono, ref T cache) where T : Component
        {
            if (!cache) cache = mono.GetComponentInParent<T>();
            return cache;
        }
    }
    ...
    public static class GameObjectExtension
    {
        ...
        {
            if (!cache) cache = go.GetComponent<T>();
            return cache;
        }

        public static T GetAndCacheComponentInChildren<T>(this GameObject go, bool includeInactive, ref T cache) where T : Component
        {
            if (!cache) cache = go.GetComponentInChildren<T>(includeInactive);
            return cache;
        }
    }
